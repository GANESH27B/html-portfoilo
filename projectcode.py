...
from twilio.rest import Client  # ✅ SMS Integration
...
class BloodSugarMonitor(QWidget):
    def __init__(self):
        ...
        # Twilio Setup (replace with your actual values)
        self.twilio_sid = "AC3b7e76e20045380312cc6e53ecf5ae6b"
        self.twilio_auth = "35dfbb9e28514250fa5e40e3069580c4"
        self.twilio_from = "+12253402758"  # Your Twilio number
        self.user_phone = "+919705711785"   # Your phone number
        self.sms_client = Client(self.twilio_sid, self.twilio_auth)
        self.sms_sent_max = False
        self.sms_sent_min = False
    ...
    def update_data(self):
        ...
        # SMS Alert for max/min
        if value == max(self.blood_sugar_values) and not self.sms_sent_max:
            self.send_sms(f"🚨 Highest Blood Sugar Today: {value} mg/dL at {timestamp}")
            self.sms_sent_max = True

        if value == min(self.blood_sugar_values) and not self.sms_sent_min:
            self.send_sms(f"⚠️ Lowest Blood Sugar Today: {value} mg/dL at {timestamp}")
            self.sms_sent_min = True
